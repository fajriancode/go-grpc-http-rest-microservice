...
import (
	...
	v1 "github.com/fajriancode/go-grpc-http-rest-microservice/pkg/api/v1"
...
func (s *toDoServiceServer) Update(ctx context.Context, req *v1.UpdateRequest) (*v1.UpdateResponse, error) {
	...
	if err != nil {
		return nil, status.Error(codes.Unknown, "failed to retrieve rows affected value from ToDo->"+err.Error())
	}
...
func (s *toDoServiceServer) ReadAll(ctx context.Context, req *v1.ReadAllRequest) (*v1.ReadAllResponse, error) {
	...
	for rows.Next() {
		td := new(v1.ToDo)
		if err := rows.Scan(&td.Id, &td.Title, &td.Description, &reminder); err != nil {
			return nil, status.Error(codes.Unknown, "failed to retrieve field value from ToDo->"+err.Error())
		}
